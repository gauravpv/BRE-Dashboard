...
class PageControllerTest {
    ...
    void dashboard() throws Exception {
        ...
        mockMvc.perform(get("/").with(user(BreUserPrincipal.fromUser(admin))))
    ...
    void systemAdmin_rendersSeededServices() throws Exception {
        AppUser admin = userRepository.findByAzureAdId("dev-admin").orElseThrow();
        mockMvc.perform(get("/admin").with(user(BreUserPrincipal.fromUser(admin))))
                .andExpect(status().isOk())
    ...
    void developerUtils_rendersPage() throws Exception {
        AppUser admin = userRepository.findByAzureAdId("dev-admin").orElseThrow();
        mockMvc.perform(get("/dev-utils").with(user(BreUserPrincipal.fromUser(admin))))
                .andExpect(status().isOk())
